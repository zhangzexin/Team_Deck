...
import com.zzx.desktop.teamdeck.socket.ssl.TrustAllCerts
 ...
 * in sequence.
 */
class CMockWebServer : ExternalResource(), Closeable {
    ...
    private fun acceptConnections() {
        while (true) {
            ...
            try {
                ...
                logger.fine("${this@CMockWebServer} done accepting connections: ${e.message}")
                return
            }
    ...
    internal inner class SocketHandler(private val raw: Socket) {
        ...
        fun handle() {
            ...
            if (sequenceNumber == 0) {
                logger.warning(
                    "${this@CMockWebServer} connection from ${raw.inetAddress} didn't make a request")
            }
        ...
        private fun processOneRequest(
            ...
            if (logger.isLoggable(Level.FINE)) {
                logger.fine(
                    "${this@CMockWebServer} received request: $request and responded: $response")
            }
    ...
    private inner class Http2SocketHandler constructor(
        ...
        override fun onStream(stream: Http2Stream) {
            ...
            if (logger.isLoggable(Level.FINE)) {
                logger.fine(
                    "${this@CMockWebServer} received request: $request " +
    ...
    companion object {
        ...
        private val UNTRUSTED_TRUST_MANAGER = TrustAllCerts()
